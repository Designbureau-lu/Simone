...
- The camera, project positions, and artwork coordinates remain global.
...
- A sampled column uses the same arc equations, source pixels, shading, and
  ...
  is unchanged.
- `public/SIMONE-export/SIMONE-projects.txt` establishes the ordered project
  metadata and cumulative variable-width artwork before decoding. Only projects
  required by the initial viewport and guard region block the first curtain;
  remaining project images load through the same catalog-backed artwork in the
  background. Queued loading follows Pan, predicted camera inertia, and project
  destinations without changing camera or interaction behavior.
